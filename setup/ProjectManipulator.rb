...
module Pod
  ...
  class ProjectManipulator
    ...
    def run
      @string_replacements = {
        "BeforeUsingCoordinator_OWNER" => @configurator.user_name,
        "TODAYS_DATE" => @configurator.date,
        "TODAYS_YEAR" => @configurator.year,
        "BeforeUsingCoordinator" => @configurator.pod_name,
        "CPD" => @prefix
      }
    ...
    def add_podspec_metadata
      ...
      project_metadata_item.new_file "../BeforeUsingCoordinator.podspec"
    ...
    def remove_demo_project
      ...
      `rm -rf templates/ios/Example/BeforeUsingCoordinator`
      `rm -rf templates/swift/Example/BeforeUsingCoordinator`
...
RUBY
    ...
    def rename_files
      ...
      scheme_path = project_folder + "/BeforeUsingCoordinator.xcodeproj/xcshareddata/xcschemes/"
      File.rename(scheme_path + "BeforeUsingCoordinator.xcscheme", scheme_path +  @configurator.pod_name + "-Example.xcscheme")

      # rename xcproject
      File.rename(project_folder + "/BeforeUsingCoordinator.xcodeproj", project_folder + "/" +  @configurator.pod_name + ".xcodeproj")

      unless @remove_demo_target
        # change app file prefixes
        ["CPDAppDelegate.h", "CPDAppDelegate.m", "CPDViewController.h", "CPDViewController.m"].each do |file|
          before = project_folder + "/BeforeUsingCoordinator/" + file
          next unless File.exists? before

          after = project_folder + "/BeforeUsingCoordinator/" + file.gsub("CPD", prefix)
          File.rename before, after
        end

        # rename project related files
        ["BeforeUsingCoordinator-Info.plist", "BeforeUsingCoordinator-Prefix.pch", "BeforeUsingCoordinator.entitlements"].each do |file|
          before = project_folder + "/BeforeUsingCoordinator/" + file
          next unless File.exists? before

          after = project_folder + "/BeforeUsingCoordinator/" + file.gsub("BeforeUsingCoordinator", @configurator.pod_name)
          File.rename before, after
        end
      end
    ...
    def rename_project_folder
      if Dir.exist? project_folder + "/BeforeUsingCoordinator"
        File.rename(project_folder + "/BeforeUsingCoordinator", project_folder + "/" + @configurator.pod_name)
      end
    end
